...
typedef struct {
    ...
    uint32_t bg_color;              // Background color RRGGBBA
    ...
    extension_t current_extension;  // Current extension support e.g. CHIP8 vs. SUPERCHIP
} config_t;
...
void audio_callback(void *userdata, uint8_t *stream, int len) {
    ...
    // We are filling out 2 bytes at a time (int16_t), len is in bytes, so divide by 2
        ...
        switch (event.type) {
            ...
            case SDL_KEYDOWN:
                switch (event.key.keysym.sym) {
                    ...
                    case SDLK_SPACE:
                        // Space bar: Pause/Resume emulation state
